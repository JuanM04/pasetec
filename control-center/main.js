...
ipcMain.on('get-user', (e, data) => {
  post('/get-user', data)
    .then(user => {
      ...
      else e.reply('status', { type: 'ERROR', message: 'El usuario no se ha encontrado' })
    })
})

ipcMain.on('add-viajes', (e, data) => {
  post('/add-viajes', data)
    .then(res => {
      if(!res || res.error) e.reply('status', { type: 'ERROR', message: res.error || false })
...
ipcMain.on('get-prices', (e, data) => {
  post('/get-prices', data)
    .then(metadata => {
      if(metadata) e.reply('get-prices-res', metadata)
      else e.reply('status', { type: 'ERROR', message: 'Error al cargar los precios' })
    })
})

ipcMain.on('update-prices', (e, data) => {
  post('/update-prices', data)
    .then(metadata => {
      e.reply('status', {
